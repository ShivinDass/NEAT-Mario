...
#########################################################################################
    ...
    def getTile(self, dx, dy):
        x = self._x_position + dx + 8
        y = self._y_position + dy - 16
    ...
    def getSprites(self):
        ...
        for slot in range(0,4):
            enemy = self._read_mem(0xF+slot)   #memory.readbyte(0xF+slot)
            if not enemy == 0:
                ex = self._read_mem(0x6E + slot)*0x100 + self._read_mem(0x87+slot)
                ey = self._read_mem(0xCF + slot)+24
    ...
    def getInputs(self):
        ...
        BoxRadius=6
        for dy in range(-16*BoxRadius,16*BoxRadius,16):   #=-BoxRadius*16,BoxRadius*16,16 do
            for dx in range(-16*BoxRadius,16*BoxRadius,16):
                inputs.append(0)#[#inputs+1] = 0
                       
                tile = self.getTile(dx, dy)
                if tile == 1 and self._y_position+dy < 0x1B0:
                    inputs[len(inputs)-1]=1#[#inputs] = 1
                       
                for i in range(0,len(sprites)):#= 1,#sprites do
